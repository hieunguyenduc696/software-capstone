...
async function testCreateParagraphs(
  request: Request,
  response: Response
): Promise<any> {
...
//Dummy version for testing database work correctly
async function testUpdateParagraph(
  request: Request,
  response: Response
): Promise<any> {
...
//Dummy version for testing database work correctly
async function testDeleteParagraphs(
  request: Request,
  response: Response
): Promise<any> {
  const startId = 0;
...
async function createReadingTests(
  request: Request,
  response: Response
): Promise<any> {
...
async function pagingReadingTests(request: Request, response: Response) {
  ...
  const limit: number = request.query.limit
    ...
    : DEFAULT_PAGING_LIMIT;
  const pageIndex: number = request.query.page
    ? parseInt(String(request.query.page))
    : DEFAULT_PAGING_INDEX;
